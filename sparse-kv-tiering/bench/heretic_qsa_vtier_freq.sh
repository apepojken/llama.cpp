...
OUT=$BD/heretic_qsa_vtier_freq.out; PORT=8097
...
for spec in $FRACS; do
  ...
  envs=(); [ "$frac" != none ] && envs=(LLAMA_QSA_VTIER=$frac LLAMA_QSA_VTIER_PREFILL=$pf LLAMA_QSA_VTIER_POLICY=${POLICY:-recency})
  log "== hot tier: $frac (restricts prefill too: $pf, policy ${POLICY:-recency}) =="
  env "${envs[@]}" "$NEW/llama-server" --host 127.0.0.1 --port $PORT -m "$MODEL" --mmproj "$MMPROJ" --device Vulkan0 -ngl 999 \
     ...
     --load-mode mmap --lazy-mode on --cache-reuse 256 --cache-ram 0 --ctx-checkpoints 4 > "$BD/vt_srv_${frac}_p${pf}_${POLICY:-recency}.log" 2>&1 &
  PID=$!; ok=0
...
PY
  grep -E "qsa_vtier: (filter|step [0-9]+:)" "$BD/vt_srv_${frac}_p${pf}_${POLICY:-recency}.log" | sed 's/^.*W *//;s/^/    /' | sort -u | head -4 | tee -a "$OUT"
