...
ggml_tensor * llama_model_qwen4exp::graph::build_hc_mix(
    ...
    if (inject) {
        ...
        // the layout.
        ggml_tensor * w;
        // batched prefill fast path: compute the transpose so the m=4 projection rides the
        // mat-vec shaders instead of the near-empty tiled path. Only valid when w_inject is
        // unquantized, though: the transpose puts w_inject on the mul_mat B side, which the
        // Vulkan mat-vec accepts only as f32/f16/q8_1 (a quantized weight there asserts).
        // For a quantized inject weight, keep the normal weight-as-A path.
...
public:
    ...
    void set_input(const llama_ubatch * ubatch) override {
        ...
        mctx->set_input_qsa(cell_blk, blk_cells, blk_pos, bias, ubatch, ratio, blk_bias,
                dirty_cells, dirty_pos, dirty_rows, tail_cells, tail_mask);
    }
    ...
    bool can_reuse(const llm_graph_params & params) override {
        ...
        // [TAG_QSA_POOLED_CACHE] the dirty tables must hold this ubatch's (re)pool range; a
        // 0 from the capacity check means the pooled path must be left, which also rebuilds
        if (dirty_rows != nullptr) {
            res &= dirty_rows->ne[0] == (int64_t) mctx->qsa_pooled_n_dirty_max(params.ubatch, ratio);
        }

        // [TAG_QSA_GATHER] the tail rows are per query; a batch size or n_kv change that would
        // flip the gather decision already fails the k_idxs / blk_cells checks above
    ...
    ggml_tensor * cell_blk  = nullptr;   // I32 [n_kv, n_stream]            (per-cell selection only)
    ggml_tensor * blk_cells = nullptr;   // I32 [ratio*n_blocks, n_stream]
    ggml_tensor * blk_pos   = nullptr;   // I32 [4*n_blocks*n_stream]       (full recompute only)
    ...
    // [TAG_QSA_GATHER] present only on the gather path: the tail cells of every query and their
    // visibility (0 / -inf). the spare block stays out of the block-level selection there, since
    // its repeated members would be attended twice in a gathered row list.
    ggml_tensor * tail_cells = nullptr;  // I32 [ratio, n_tokens]
    ggml_tensor * tail_mask  = nullptr;  // F32 [ratio, n_tokens]
    int64_t       n_sel      = 0;        // padded row count of the gathered attention
    int64_t       n_valid    = 0;        // rows that carry the selection (blocks + tail); the rest pad
...
ggml_tensor * llama_model_qwen4exp::graph::build_qsa_top_k(
    ...
    if (it != qsa_inps.end()) {
        ...
        // pooled/normed/roped, and the score reads the cache. blk_pos is then a dead input
        // (the positions travel in dirty_pos), so it is not created. Kill switch for A/B tests.
        const uint32_t n_dirty_max =
            (mctx_hyb->get_pooled_k(il) != nullptr && n_stream == 1 && getenv("LLAMA_QSA_NO_POOLED_CACHE") == nullptr)
                ? mctx_hyb->qsa_pooled_n_dirty_max(ubatch, (uint32_t) r) : 0;

        qsa->k_idxs    = mctx_idx->build_input_k_idxs(ctx0, ubatch);
        // the block-level selection never expands per cell, so it has no use for the cell -> block map
        // (an input nothing consumes gets no buffer, so it must not exist at all)
        qsa->cell_blk  = blk_bias ? nullptr : ggml_new_tensor_2d(ctx0, GGML_TYPE_I32, n_kv, n_stream);
        qsa->blk_cells = ggml_new_tensor_2d(ctx0, GGML_TYPE_I32, r*n_blocks, n_stream);
        qsa->blk_pos   = n_dirty_max > 0 ? nullptr : ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, 4*n_blocks*n_stream);
        ...
        // [TAG_QSA_GATHER] decode fast path: attend over the gathered selection instead of
        // masking all n_kv cells. the row count is padded to the flash attention granularity;
        // the tail rows and their visibility come from set_input
        if (blk_bias && n_stream == 1) {
            const int64_t width = std::min<int64_t>(n_kv, (int64_t) hparams.indexer_top_k + r - 1);
            const int64_t k_blk = std::min<int64_t>(n_blocks, (width + r - 1)/r);
            const int64_t n_sel = qsa_gather_n_sel(n_kv, r*k_blk + r);

            if (n_sel > 0) {
                qsa->n_sel      = n_sel;
                qsa->n_valid    = r*k_blk + r;
                qsa->tail_cells = ggml_new_tensor_2d(ctx0, GGML_TYPE_I32, r, n_tps);
                qsa->tail_mask  = ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, r, n_tps);
    ...
    if (inp->dirty_rows != nullptr) {
        ...
        // the cache; the score then reads the cache. Rows of blocks not pooled yet hold stale
        // (finite) data and are masked by the -inf bias, like the partial pools of the full
        // recompute below. set_rows is expanded before the score node that reads the store, so
        // the graph keeps that order.
        ...
        for (int64_t i = 0; i < r; ++i) {
            ...
            // r >= 2, so the first add materializes the contiguous sum without a copy
    ...
    if (blk_bias && n_stream == 1) {
        // [TAG_QSA_BLOCK_SELECT] select whole blocks, then expand the winners to their cells.
        // every cell of a block carries the block score, so a top-k over the cells is a top-k
        // over the blocks; taking it per block touches n_blocks elements instead of n_kv and
        // needs no per-cell expansion, mask cast or add. the budget is whole blocks: the reference
        // width covers indexer_top_k/r full blocks plus the tail, and the tail travels in the
        // spare block (bias 1e9), whose member row set_input fills with the unpooled cells,
        // padded with repeats. repeats are harmless: build_attn_qsa unmasks rows, it does not
        // gather them. cells behind a -inf mask (future, foreign) stay masked there as well.
        //
        // [TAG_QSA_SCORE_CHUNK] a prefill ubatch scores n_blocks x n_idx_h x n_tokens floats per
        // layer (2 GiB at 256k with 2048 tokens), and that is what sizes the compute scratch.
        // the math is per query column, so score and select in chunks of queries: the allocator
        // then keeps one chunk live instead of the whole batch. LLAMA_QSA_SCORE_CHUNK tunes it.
        static const int64_t chunk = [] {
            const char * env = getenv("LLAMA_QSA_SCORE_CHUNK");
            const int64_t v = env ? atoll(env) : 512;
            return v <= 0 ? INT64_MAX : v;
        }();

        // blk_cells [r*n_blocks, 1] -> one row of r cells per block
        ggml_tensor * blk_cells = ggml_reshape_3d(ctx0, inp->blk_cells, r, n_blocks, 1);

        ggml_tensor * cells_all = nullptr;

        for (int64_t c0 = 0; c0 < n_tps; c0 += chunk) {
            const int64_t c = std::min<int64_t>(chunk, n_tps - c0);

            // the chunk's queries: a column range of q, one contiguous slab
            ggml_tensor * q_c = ggml_view_3d(ctx0, q, idx_dim, n_idx_h, c, q->nb[1], q->nb[2], c0*q->nb[2]);

            // rectify each head dot product before the sum, as in the DeepSeek lightning indexer
            ggml_tensor * s = ggml_mul_mat(ctx0, pooled, ggml_reshape_3d(ctx0, q_c, idx_dim, n_idx_h*c, 1));
            s = ggml_reshape_4d(ctx0, s, n_blocks, n_idx_h, c, 1);
            s = ggml_relu(ctx0, s);

            // the heads sit side by side on ne[1] and there are only a few of them
            ggml_tensor * sum = nullptr;
            for (int64_t h = 0; h < n_idx_h; ++h) {
                ggml_tensor * slice = ggml_view_3d(ctx0, s, n_blocks, c, 1, s->nb[2], s->nb[3], h*s->nb[1]);
                sum = sum ? ggml_add(ctx0, sum, slice) : ggml_cont(ctx0, slice);
            }

            // one value per block, so it is cheaper to bias here than after the cells are
            // expanded; the chunk's bias columns are one contiguous slab of the input
            ggml_tensor * b_c = ggml_view_3d(ctx0, inp->bias, n_blocks, c, 1,
                    inp->bias->nb[1], inp->bias->nb[2], c0*inp->bias->nb[1]);
            sum = ggml_add(ctx0, sum, b_c);
            cb(sum, "indexer_score", il);

            ggml_tensor * top_blk = ggml_top_k(ctx0, sum, k_blk);
            cb(top_blk, "indexer_top_blk", il);

            ggml_tensor * cells = ggml_get_rows(ctx0, blk_cells, ggml_reshape_2d(ctx0, top_blk, k_blk*c, 1));
            cells = ggml_reshape_3d(ctx0, cells, r*k_blk, c, 1);

            cells_all = cells_all ? ggml_concat(ctx0, cells_all, cells, 1) : cells;
        }

        // build_attn_qsa reads [n_top_k, n_batch, 1, n_stream], matching the KQ mask.
        top_k = ggml_reshape_4d(ctx0, cells_all, r*k_blk, n_tps, 1, 1);

        // [TAG_QSA_GATHER] the gathered attention wants a row list: the block cells, then each
        // query's tail rows (masked per slot by set_input), then pads up to the flash attention
        // granularity. pads repeat the first index and are masked out again in the gather.
        if (inp->tail_cells != nullptr) {
            GGML_ASSERT(n_stream == 1);

            ggml_tensor * tail = ggml_reshape_4d(ctx0, inp->tail_cells, r, n_tps, 1, 1);
            top_k = ggml_concat(ctx0, top_k, tail, 0);
            GGML_ASSERT(top_k->ne[0] == inp->n_valid);

            if (inp->n_sel > inp->n_valid) {
                ggml_tensor * first = ggml_view_4d(ctx0, top_k, 1, n_tps, 1, 1,
                        top_k->nb[1], top_k->nb[2], top_k->nb[3], 0);
                ggml_tensor * pad = ggml_repeat_4d(ctx0, first, inp->n_sel - inp->n_valid, n_tps, 1, 1);
        ...
        if (blk_bias) {
            // multi-stream: the block-level selection over every stream at once
            score = ggml_add(ctx0, score, inp->bias);

            ggml_tensor * top_blk = ggml_top_k(ctx0, score, k_blk);
            cb(top_blk, "indexer_top_blk", il);

            // blk_cells [r*n_blocks, n_stream] -> one row of r cells per block; get_rows batches
            // over ne[2], so the winners of stream s expand through the cell rows of stream s
            ggml_tensor * blk_cells = ggml_reshape_3d(ctx0, inp->blk_cells, r, n_blocks, n_stream);
            ggml_tensor * cells     = ggml_get_rows(ctx0, blk_cells,
                    ggml_reshape_2d(ctx0, top_blk, k_blk*n_tps, n_stream));

            // build_attn_qsa reads [n_top_k, n_batch, 1, n_stream], matching the KQ mask.
            top_k = ggml_reshape_4d(ctx0, cells, r*k_blk, n_tps, 1, n_stream);
        } else {
...
// [TAG_QSA_GATHER] ported from the qwen4exp-spec-mtp tree (there from EngramHalo.cpp).
// Decides whether build_attn_qsa may gather the selected rows instead of masking all n_kv cells.
// Returns the padded row count for the gathered attention, or 0 to keep the masked path.
int64_t llama_model_qwen4exp::graph::qsa_gather_n_sel(int64_t n_kv, int64_t n_valid) const {
    // opt-out / tuning: LLAMA_QSA_GATHER=0 disables the gather, an integer sets the minimum n_kv
    // it activates at; "1" forces it on for testing. Default 32768: below that the masked path
    // costs about the same (measured on RADV: -1% at 16k, tie at 32k, +2% at 64k, growing)
    ...
    // masked path is the only one exercised with it, so do not diverge from it here.
    if (!cparams.flash_attn || hparams.use_alibi) {
        return 0;
    }

    // decode-sized batches only: a prefill ubatch amortizes the masked pass over many queries,
    // while the gather cost scales with n_tokens (each token gathers and casts its own n_sel
    // rows). measured on RADV with this tree: one query wins past ~30k, a 7-query speculative
    // verify batch loses at 56k (19.7 vs 21.9 t/s), so the threshold scales with the batch:
    // n_kv >= min_kv * n_tokens (default 32768 per query -> 7 queries need ~230k)
    if (n_tokens > 16 || n_kv < min_kv * n_tokens) {
        return 0;
    }

    // FATTN_KQ_STRIDE: the fastest kernels need the KV length padded to 256
    const int64_t n_sel = GGML_PAD(n_valid, 256);
...
// [TAG_QSA_GATHER] ported from the qwen4exp-spec-mtp tree.
// Decode fast path for QSA: instead of unmasking the selected cells inside a dense [n_kv]
// attention, gather exactly those K/V rows out of the cache and attend over [n_sel] rows. KV
// bandwidth and attention work per token drop from O(n_kv) to O(n_sel). The result matches the
// masked path:
//   - rows [0, n_valid) carry the selection: the block cells, then the query's tail rows; rows
//     [n_valid, n_sel) only pad the count to the flash attention granularity and are masked out
//   - every block row keeps its original visibility: mask_sel[j] = kq_mask[top_k[j]]; the tail rows
//     take the per-slot visibility set_input wrote (a pad slot there is -inf)
...
ggml_tensor * llama_model_qwen4exp::graph::build_attn_qsa_gather(
        ...
        ggml_tensor * top_k,    // I32 [n_sel, n_tps, 1, ns] cell indices
        const llm_graph_input_qsa * qsa,
        float         kq_scale,
        int           il) {
    ...
    const int64_t n_valid = qsa->n_valid;
    const int64_t n_tail  = qsa->tail_mask->ne[0];
    const int64_t n_blk   = n_valid - n_tail;

    GGML_ASSERT(ns == 1 && "the gathered QSA attention is single-stream only");
    GGML_ASSERT(top_k->ne[3] == ns);
    GGML_ASSERT(nt == q_cur->ne[2]);
    GGML_ASSERT(n_sel == qsa->n_sel && n_valid <= n_sel && n_sel <= n_kv);
    ...
    // every token's indices in one flat list
    ...
    // [d*hkv, n_sel*n_tps, ns] -> [d, n_sel, hkv, nt]: token t is ne3 slice t, the ubatch order
    ...
    // per-row visibility. block rows: gather each selected cell's original mask value (rows of
    // size 1). tail rows: the per-slot visibility from set_input. pad rows: -inf.
    ggml_tensor * idx_b = ggml_view_3d(ctx0, top_k, n_blk, n_tps, ns, top_k->nb[1], top_k->nb[3], 0);
    ggml_tensor * m_sel = ggml_get_rows(ctx0,
            ggml_reshape_4d(ctx0, kq_mask, 1, n_kv, n_tps, ns), idx_b);        // F32 [1, n_blk, n_tps, ns]

    ggml_tensor * m_tail = ggml_reshape_4d(ctx0, qsa->tail_mask, 1, n_tail, n_tps, 1);
    m_sel = ggml_concat(ctx0, m_sel, m_tail, 1);

    if (n_valid < n_sel) {
        ggml_tensor * m_pad = ggml_new_tensor_4d(ctx0, GGML_TYPE_F32, 1, n_sel - n_valid, n_tps, ns);
...
ggml_tensor * llama_model_qwen4exp::graph::build_attn_qsa(
    ...
    // [TAG_QSA_GATHER] decode fast path: gather the selected rows instead of masking all n_kv
    {
        const auto it = qsa_inps.find((uint32_t) hparams.dsv4_compress_ratios[il]);
        const llm_graph_input_qsa * qsa = it != qsa_inps.end() ? it->second : nullptr;

        if (qsa != nullptr && qsa->tail_mask != nullptr) {
            ggml_tensor * k = mctx_cur->get_k(ctx0, il);
            ggml_tensor * v = mctx_cur->get_v(ctx0, il);

            ggml_tensor * cur = build_attn_qsa_gather(k, v, kq_mask, q_cur, top_k, qsa, kq_scale, il);
            cb(cur, "kqv_out", il);
    ...
    // the selected cells take their original mask values (0, or the causal/sequence -inf), read
    // with one small gather: this replaces a zero fill plus an n_kv-wide add of the mask, and it
    // keeps future and foreign cells masked even when the selection names them
    // kq_mask [n_kv, n_batch, 1, n_stream] -> rows of size 1: [1, n_kv, n_batch, n_stream]
    ...
    // ggml_get_rows([1, n_kv, n_batch, n_stream], [n_top_k, n_batch, n_stream, 1]) -> f32 [1, n_top_k, n_batch, n_stream]
    ggml_tensor * vals = ggml_get_rows(ctx0, kq_mask_rows, top_k_3d);

    // modify KQ mask by unmasking elements that are in top_k indices
    // ggml_set_rows([1, n_kv, n_batch, n_stream], [1, n_top_k, n_batch, n_stream], [n_top_k, n_batch, n_stream, 1])
    ggml_tensor * kq_mask_top_k = ggml_set_rows(ctx0, kq_mask_all, vals, top_k_3d);
